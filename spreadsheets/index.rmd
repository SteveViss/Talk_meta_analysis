...
# Scenario 2. Each collaborator works on _different spreadsheets_
...
### In a 3-4 collaborators context...
...
```r
# create a vector contains the path to all files
files <- list.files('path/to/folder', pattern='^F.*csv$', full.names=TRUE)

# Create a empty list
ls_csv <- list()

# The loop populates each level of the list with the content of each files
for (i in 1:length(files)){
  print(files[i])
  ls_csv[[i]] <- read.csv(files[i])
}

# We finaly apply the function rbind at each level of the list
